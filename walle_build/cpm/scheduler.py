...
def initialize():
    ...
    logger.main('system','INITIALIZING SCHEDULER')
    # logger.main('scheduler','INITIALIZING SCHEDULER')
    SCHEDULER_THREAD=threading.Thread(target=main,args=(),daemon=False,name="scheduler")
    SCHEDULER_THREAD.start()

def clean_up():
    SCHEDULER_HALT.set()
    signals.vc_start.clear()
    signals.ac_start.clear()
