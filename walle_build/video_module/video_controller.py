...
def clean_up():
    ...
    while not R2C.empty():
        try:_=R2C.get_nowait()
        except queue.Empty:break
    while not A2C.empty():
        try:_=A2C.get_nowait()
        except queue.Empty:break
    print("VIDEO CONTROLLER TERMINATED")

def write_to_file(MESSAGE):
    with open(DEBUG_FILE,"a") as FILE:FILE.write(MESSAGE)

# main
def main(VC2C):
    global ACTIVE
    with open(DEBUG_FILE,"w") as FILE:FILE.write("")
    ...
    while True:
        ...
        if elapsed>=ANALYSIS_CYCLE:
            ...
            BUFFER.clear()
            ACTIVE=True
        elif elapsed>=ANALYSIS_WINDOW:ACTIVE=False
        if CONTROLLER_STOP.is_set():break
        try:FRAME=R2C.get(timeout=0.2)
        except queue.Empty:continue
        try:C2A.put(FRAME,timeout=0.2)
        ...
        if ACTIVE:
            ...
            if not CONTROLLER_STOP.is_set():
                try:
                    if len(BUFFER)==0:VC2C.put({"BUFFER":None,"SOURCE":1,"TIMESTAMP":CYCLE_START},block=False)
                    else:VC2C.put({"BUFFER":BUFFER.copy(),"SOURCE":1,"TIMESTAMP":CYCLE_START},block=False)
